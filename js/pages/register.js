...
function validarPaso(paso) {
    if (paso == 1) {
        ...
        if (!nombre || !email || !password || !confirmPassword || !pais) {
            ChichenItzaNotifications.error('Por favor completa todos los campos obligatorios');
            return false;
        }
        ...
        if (password.length < 8) {
            ChichenItzaNotifications.error('La contraseña debe tener al menos 8 caracteres');
            return false;
        }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    form.addEventListener('submit', function(e) {
        ...
        const sesion = {
            ...
            email: datos.email,
            nombre: datos.nombre,
            nombreAvatar: datos.nombreAvatar,
            nivel: datos.membresia,
            avatarImage: datos.avatarImage,
            fechaRegistro: datos.fechaRegistro
        };
        localStorage.setItem('sesionActual', JSON.stringify(sesion));

        // Show success message
        ChichenItzaNotifications.success('¡Registro exitoso! Bienvenido a Chichen Itzá Virtual Tours');
